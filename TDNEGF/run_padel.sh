...
n_size_values=(2 4 6 8 10 12)

for n_size in "${n_size_values[@]}"; do
    echo "Running simulation with n=$n_size, t_end=$t_end..."
    output_file="$OUTPUT_DIR/padel_nsize_${n_size}_${t_end}.txt"

    # Run Julia and capture both stdout and stderr (timing info)
    # Use || true to prevent early exit if Julia returns non-zero
    if ! julia "$script_path" "$n_size" "$t_end" > "$output_file" 2>&1; then
